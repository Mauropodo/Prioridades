import { Lightbulb } from 'lucide-react';
import { Button } from './ui/button';

interface LoginScreenProps {
  onLogin: () => void;
}

export function LoginScreen({ onLogin }: LoginScreenProps) {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white px-8">
      <div className="flex flex-col items-center space-y-6 max-w-md w-full">
        {/* Logo */}
        <div className="flex items-center justify-center w-24 h-24 bg-gradient-to-br from-sky-400 to-sky-600 rounded-3xl shadow-lg">
          <Lightbulb className="w-12 h-12 text-white" />
        </div>

        {/* Title */}
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-semibold text-slate-800">IdeaFlow</h1>
          <p className="text-slate-500">
            Tu espacio para registrar ideas y medir tu progreso
          </p>
        </div>

        {/* Google Sign In Button */}
        <div className="w-full pt-8">
          <Button
            onClick={onLogin}
            className="w-full h-14 bg-sky-500 hover:bg-sky-600 text-white rounded-xl shadow-md transition-all duration-200"
            bravo-auth-login-google="true"
          >
            <svg className="w-6 h-6 mr-3" viewBox="0 0 24 24">
              <path
                fill="currentColor"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="currentColor"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="currentColor"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="currentColor"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            Continuar con Google
          </Button>
        </div>

        {/* Footer text */}
        <p className="text-xs text-slate-400 text-center pt-4 px-4">
          Al continuar, aceptas nuestros términos de servicio y política de privacidad
        </p>
      </div>
    </div>
  );
}
